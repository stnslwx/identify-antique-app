...
struct CollectionItemView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 16) {
                    VStack(spacing: 16) {
                        CollItemTopBar(itemName: "Hoff Sofa", isConfirmational: isConfirmationalView)
                        ItemImageView()
    ...
    struct CollItemTopBar: View {
        ...
        var body: some View {
            HStack {
                ItemInfoButton(action: {print("go back")}, icon: "arrowLeft", isSecondary: true)
                Spacer()
                Text(itemName).font(.system(size: 20, weight: .bold))
                Spacer()
                if isConfirmational {
                    ItemInfoButton(action: {print("confirm")}, icon: "tick", isSecondary: false)
                } else {
                    ItemInfoButton(action: {print("confirm")}, icon: "plus", isSecondary: false)
                }
...
#Preview {
    CollectionItemView(isConfirmationalView: false)
}
